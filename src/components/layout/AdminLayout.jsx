...
import {
  ...
  CalendarDays, Banknote, LogOut, Building2, Menu, X, ChevronRight
} from 'lucide-react'

const nav = [
  { to: '/admin',             label: 'Dashboard',   icon: LayoutDashboard, end: true },
  { to: '/admin/leads',       label: 'Lead',        icon: Users },
  { to: '/admin/agents',      label: 'Ejen',        icon: UserCheck },
  { to: '/admin/projects',    label: 'Projek',      icon: FolderKanban },
  { to: '/admin/followups',   label: 'Followup',    icon: CalendarDays },
  { to: '/admin/commissions', label: 'Komisyen',    icon: Banknote },
]
      ...
      `}>
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-navy-700">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gold-400 truncate">Admin Panel</p>
          </div>
        </div>
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors
                 ${isActive
                   ...
                   : 'text-gray-400 hover:text-gray-100 hover:bg-navy-700/60'}`
              }
            >
